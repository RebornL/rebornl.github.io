...
![spring4.0](https://raw.githubusercontent.com/RebornL/picbed/master/springkuangjia.png)

Spring Hello World实例：

首先定义一个HelloWorld类，有message属性以及相关的setter和getter方法，然后在MainApp文件中使用`ClassPathXmlApplicationContext(“xxx.xml”)`创建应用程序的上下文，创建初始化beans的配置文件所有的对象，之后可以通过getBean方法获取所需要的bean，根据bean的ID获取对象。在xml配置文件创建自己需要的实例化对象，从而不需要修改HelloWorld.java和MainApp.java文件。



Spring的`BeanFactory`容器：常用在移动设备和applet的应用中,最常用的`XmlBeanFactory`，通过xml加载初始化bean
...
![scope](https://raw.githubusercontent.com/RebornL/picbed/master/scope-property.png)
...
以上的xml配置，均为显示注入依赖，Beans还可以通过指定bean的autowire属性为一个bean定义一个自动装配的模式：![beans模式](https://raw.githubusercontent.com/RebornL/picbed/master/bean-way.png)
...
整体框架围绕DispatcherServlet设计，![1554684539358](https://raw.githubusercontent.com/RebornL/picbed/master/1554684539358.png)
